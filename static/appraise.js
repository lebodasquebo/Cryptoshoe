let state={market:[],hold:[],appraised:[],hist:{},balance:0,next_stock:0,server_time:0},es,serverOffset=0
...
const card=(h,i)=>{
  ...
  d.innerHTML=`
    <div class="appraise-card-head">
      <div class="appraise-card-name"></div>
      <div class="appraise-card-rarity"></div>
    </div>
    <div class="appraise-card-qty"></div>
    <div class="appraise-card-price"></div>
    <div class="appraise-card-cost"></div>
    <div class="appraise-qty-select">
      <label>Appraise:</label>
      <div class="appraise-qty-wrap">
        <button class="qty-btn qty-dec">−</button>
        <input type="number" class="appraise-qty" min="1" value="1">
        <button class="qty-btn qty-inc">+</button>
      </div>
      <button class="qty-max-btn">MAX</button>
    </div>
    <button class="appraise-btn">🔍 APPRAISE</button>
  `
...
const updCost=(d,h)=>{
  let qty=parseInt(d.querySelector('.appraise-qty').value)||1
  qty=Math.min(qty,h.qty)
  let price=h.sell_price||h.base
  let cost=price*0.05*qty
  d.querySelector('.appraise-card-cost').textContent=`Cost: $${money(cost)} (${qty}×$${money(price*0.05)})`
}
...
const showResults=(res)=>{
  let m=$('#appraise-modal')
  let content=$('#modal-results')
  content.innerHTML=''
  if(res.qty===1){
    let r=res.results[0]
    content.innerHTML=`
      <div class="single-result">
        <div class="modal-rating ${r.rating_class}">${r.rating.toFixed(1)}</div>
        <div class="modal-comment">${r.comment}</div>
        <div class="modal-mult ${r.multiplier>=1?'up':'down'}">${r.multiplier>=1?'+':''}${((r.multiplier-1)*100).toFixed(0)}% value</div>
      </div>
    `
  }else{
    let html='<div class="multi-results"><div class="results-grid">'
    res.results.forEach((r,i)=>{
      html+=`<div class="result-item ${r.rating_class}">
        <div class="result-num">#${i+1}</div>
        <div class="result-rating">${r.rating.toFixed(1)}</div>
        <div class="result-mult ${r.multiplier>=1?'up':'down'}">${r.multiplier>=1?'+':''}${((r.multiplier-1)*100).toFixed(0)}%</div>
      </div>`
    })
    html+=`</div><div class="best-result">
      <div class="best-label">BEST RESULT</div>
      <div class="best-rating ${res.best.rating_class}">${res.best.rating.toFixed(1)}</div>
      <div class="best-comment">${res.best.comment}</div>
    </div></div>`
    content.innerHTML=html
  }
  let hasPerfect=res.results.some(r=>r.perfect)
  m.classList.toggle('has-perfect',hasPerfect)
...
const upd=()=>{
  ...
  if(state.hold.length){
    ...
    cnt.textContent=state.hold.length+' shoe type'+(state.hold.length>1?'s':'')+' to appraise'
    grid.innerHTML=''
    state.hold.forEach((h,idx)=>{
      let d=card(h,idx)
      let price=h.sell_price||h.base
      d.querySelector('.appraise-card-name').textContent=h.name
      let rb=d.querySelector('.appraise-card-rarity')
      rb.textContent=h.rarity.toUpperCase()
      rb.className='appraise-card-rarity '+rarClass(h.rarity)
      ...
      d.querySelector('.appraise-qty').max=h.qty
      d.querySelector('.appraise-qty').value=1
      updCost(d,h)
      grid.append(d)
    })
  }else{
    ...
    grid.innerHTML=''
    cnt.textContent='0 shoes to appraise'
  }
}
...
const stream=()=>{
  ...
  es.onmessage=e=>{state=JSON.parse(e.data);serverOffset=state.server_time-Math.floor(Date.now()/1000);upd()}
